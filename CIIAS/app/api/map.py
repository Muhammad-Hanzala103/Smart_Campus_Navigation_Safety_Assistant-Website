...
def get_map():
    ...
    return jsonify({'nodes': [n.to_dict() for n in nodes], 'edges': [e.to_dict() for e in edges], 'map_image_url': '/static/img/campus_map.png'})

@map_bp.route('/nodes', methods=['POST'])
def create_node():
    data = request.get_json()
    node = MapNode(name=data.get('name'), x=data.get('x'), y=data.get('y'), description=data.get('description'), node_type=data.get('node_type'))
    db.session.add(node)
    db.session.commit()

...
from app import db
from app.models import FeeChallan
...
def get_challans(current_user):
    """Get all fee challans for the current user"""
    challans = FeeChallan.query.filter_by(user_id=current_user.id).order_by(FeeChallan.due_date.desc()).all()

...
class AIAnalyzer:
    ...
    def analyze_image(self, image_bytes):
        """
        Main entry point. Decides between mock and real analysis based on AI_MODE.
        """
    ...
    def analyze_with_hf(self, image_bytes):
        ...
        """
        if not self.api_key:
            return self.mock_analyze(image_bytes) # Fallback if no key
    ...
    def mock_analyze(self, image_bytes):
        ...
        """
        # Simple deterministic logic based on bytes length
        l = len(image_bytes)
        detections = []
        
        if l % 2 == 0:
            detections.append({"label": "person", "score": 0.95})
            detections.append({"label": "person", "score": 0.88})
            detections.append({"label": "backpack", "score": 0.85})
        else:
            detections.append({"label": "fire", "score": 0.99}) # Less common in random bytes, but good for testing

        return self.process_detections(detections)

    def process_detections(self, detections):
        """
        Normalizes detections and decides severity.
        """
        labels = []
        person_count = 0
        fire_detected = False
        ...
        # Handle different HF response formats or list of dicts
        if isinstance(detections, list):
            for d in detections:
                 # Check for 'label' or 'entity_group' (NER) or similar
                label = d.get('label', d.get('entity_group', 'unknown')).lower()
                score = d.get('score', 0)
                
                normalized_detections.append({"label": label, "score": score})
                labels.append(label)
        ...
        return {
            "labels": json.dumps(normalized_detections),
            "severity": severity,
